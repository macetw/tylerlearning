#originally taken from https://github.com/bbreslauer/qt-bazel-example
...
def qt_cc_library(name, src, hdr, normal_hdrs=[], deps=None, ui=None,
  ...
  native.genrule(
      ...
      cmd =  "moc $(location %s) -o $@ -f'%s'" \
        % (hdr, hdr),
   # Note, this gets confused if it's in the present directory, where the moc
   # compiler saves the file with (for example): #include "/mainwindow.h"
